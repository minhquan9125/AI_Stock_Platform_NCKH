...
IGNORE_FILES = [
    ...
    "stackexchange_qa_scraper.py",
    "vas_accounting_standards_scraper.py",
    "upload_to_gsheet.py",
...
class FinMindDataOrganizer:
    ...
    def _extract_ticker(self, filename):
        ...
        exclude = {"DATA", "INFO", "FILE", "JSON", "BCTC", "TCBS", "NEWS", "CSV", "XLSX", "TEMP", "TEST", "VAS", "QA"}
        for token in tokens:
    ...
    def _extract_category(self, filename):
        ...
        if any(
            ...
            kw in fname_lower
            for kw in ["macrocommodity", "vimo", "hanghoa", "commodity"]
        ):
...
if __name__ == "__main__":
    ...
    # "--once": quét dọn 1 lần rồi thoát (dùng cho pipeline chạy theo lịch
    # tự động, vd Windows Task Scheduler). Không truyền cờ này -> giữ
    # nguyên hành vi cũ: canh gác 24/7 trong Terminal riêng.
    if "--once" in sys.argv:
        moved = organizer.scan_and_organize()
        print(f"[*] Đã sắp xếp {moved} file (chế độ chạy 1 lần).")
        sys.exit(0)
